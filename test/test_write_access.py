...
def test_write_allowed():
	...
	for dr in ['data', 'media',]:
		assert access(join(settings.DATA_DIR, dr), W_OK | X_OK), \
			'directory "{0:s}" should be writable, but it is not'.format(join(settings.DATA_DIR, dr))
	for dr in ['lang',]:
		assert access(join(settings.BASE_DIR, dr), W_OK | X_OK), \
			'directory "{0:s}" should be writable, but it is not'.format(join(settings.BASE_DIR, dr))
...
def test_write_denied():
	...
	if settings.DEBUG:
		return
	for dr in ['static',]:
		assert not access(join(settings.DATA_DIR, dr), W_OK), \
			'directory "{0:s}" should be read-only, but it is not'.format(join(settings.DATA_DIR, dr))
	for dr in ['source', 'dev', 'docs', 'tests',]:
		assert not access(join(settings.BASE_DIR, dr), W_OK), \
			'directory "{0:s}" should be read-only, but it is not'.format(join(settings.BASE_DIR, dr))
...
def test_settings_nonworld():
	...
	"""
	pth = join(settings.BASE_DIR, 'source', 'base', 'settings_local.py')
